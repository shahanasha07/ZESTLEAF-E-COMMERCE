...
import { handleFormOpen } from '../Redux/cartSlice/addressSlice';

function CartSummary() {
    const [showAddressForm, setShowAddressForm] = useState(false);

    const { cartItems } = useSelector((state) => state.cart);
    const { isOpen } = useSelector((s) => s.address);
    console.log(isOpen);
    ...
    return (
        <div className="md:w-1/3 w-full bg-gray-50 rounded-lg p-6 shadow-sm mb-8 md:mb-0 md:ml-auto">
            ...
            <div className="flex justify-center mt-8">
                <button
                    className="bg-[#FFCB05] hover:bg-[#74B816] text-black font-semibold px-8 py-3 rounded-full shadow transition cursor-pointer"
                    onClick={handleOpen}
